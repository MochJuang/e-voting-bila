...
export default function AdminKandidat() {
  ...
  const saveJabatan = (e) => {
    e.preventDefault()
    if (jabatanModal.id) updateJabatan(jabatanModal.id, { nama: jabatanModal.nama })
    else addJabatan(jabatanModal.nama)
    setJabatanModal(null)
  }

  const saveKandidat = (e) => {
    e.preventDefault()
    const { jabatanId, id, ...form } = kandidatModal
    const patch = { ...form, nomor: Number(form.nomor) }
    if (id) updateKandidat(jabatanId, id, patch)
    else addKandidat(jabatanId, patch)
    setKandidatModal(null)
  }

  const runDelete = () => {
    if (confirmDelete.type === 'jabatan') deleteJabatan(confirmDelete.jabatanId)
    else deleteKandidat(confirmDelete.jabatanId, confirmDelete.id)
      ...
      <div className="flex flex-col gap-5">
        {jabatan.map((j) => (
          <div key={j.id} className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <button
              onClick={() => setKandidatModal({ jabatanId: j.id, ...emptyKandidat })}
