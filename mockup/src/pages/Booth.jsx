...
export default function Booth({ onSubmitted, embedded = false }) {
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      await submitVotes(currentNim ?? '2141720001', selections)
  ...
  const body = (
    <>
      ...
      {error && <p className="mt-3 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg p-2 text-center">{error}</p>}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-5 max-w-sm w-full">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleSubmit} disabled={loading} className="flex-1 rounded-lg bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white py-2 text-sm font-semibold">
