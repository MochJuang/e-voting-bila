...
export default function RegistrasiWajah() {
  ...
  const lanjut = async () => {
    ...
    setError('')
    try {
      await markFaceEnrolled(currentNim ?? '2141720001', capturedFrame || DUMMY_FRAME)
      navigate('/dashboard')
    } catch (err) {
    ...
    <Screen width="max-w-lg">
      ...
      {status === 'detected' && (
        <div className="flex flex-col gap-2">
          {error && <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg p-2 text-center">{error}</p>}
          ...
          <button onClick={lanjut} disabled={loading} className="w-full rounded-lg bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white font-semibold py-2.5">
